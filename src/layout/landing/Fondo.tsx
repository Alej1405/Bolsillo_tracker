 ...
 */
export function Fondo({ sereno = false }: { sereno?: boolean }) {
  ...
  /*
    `sereno` es el fondo del panel: el mismo papel de valores, más apagado.

    A sangre detrás de una landing el guilloché es la marca. Detrás de una
    herramienta que se usa a diario compite con los datos —y en el panel hay
    calles anchas entre tarjetas donde el dibujo queda a la vista entero. Se
    baja la opacidad del conjunto y se acorta el degradado, que era el que
    oscurecía la mitad inferior.
  */
  return (
    <div
      aria-hidden
      ...
      style={{
        opacity: sereno ? 0.42 : 1,
        background: sereno
          ? 'linear-gradient(175deg, #f8fafc 0%, #f2f5f9 45%, #e9eef5 100%)'
