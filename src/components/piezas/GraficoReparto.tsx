...
export function GraficoReparto() {
  return (
    <div className="rounded-[var(--radius-extra)] bg-fondo-superficie p-6">
      <p className="text-[20px] font-semibold text-texto-principal">En qué se fue</p>
      <p className="text-[13px] text-texto-tenue">Las cinco categorías con más gasto</p>
      <ul className="mt-5 flex flex-col gap-3.5">
        {categorias.map((c, i) => (
          <li key={c.nombre} className="flex items-center gap-3">
            ...
            <span className="w-24 shrink-0 text-[13px] text-texto-secundario">{c.nombre}</span>
            <BarraHorizontal porcentaje={c.pct} color={c.color} retraso={i * escalonado} />
            <span className="w-9 shrink-0 text-right text-[13px] text-texto-tenue tabular-nums">{c.pct}%</span>
            <CifraAnimada
              valor={c.monto}
              className="w-16 shrink-0 text-right text-[13px] font-medium text-texto-principal tabular-nums"
            />
          </li>
        ))}
      </ul>
